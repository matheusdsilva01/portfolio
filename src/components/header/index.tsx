...
const Header = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener("keypress", handleCloseMenu);
    return () => window.removeEventListener("keypress", handleCloseMenu);
  }, []);
  ...
  return (
    <header className="sticky top-0 z-10 bg-light-black/90">
      ...
      <nav className="content-wrapper flex !max-w-screen-3xl flex-row justify-between md:hidden md:py-5">
        ...
        <button
          className={`h-10 w-10 ${menuOpen ? "opacity-0" : "opacity-100"}`}
        ...
        <div
          className={`fixed right-0 top-0 flex h-full w-full max-w-xs flex-col items-center justify-center bg-light-black/90 transition-all ${
